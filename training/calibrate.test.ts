...
import { AXES, calibrate, parseLabels, TOP_KNOT_FLOOR, type LabelRow } from './calibrate.js';
...
describe('axis specs', () => {
  ...
  it('measures resolution in megapixels of the source, not the analysis plane', () => {
    const resolution = AXES.find((a) => a.name === 'resolution');
    expect(resolution?.measure(features({ width: 4000, height: 3000 }) as never)).toBeCloseTo(12, 9);
...
function dataset(
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const width = 1000;
    const height = Math.round((megapixels * 1_000_000) / width);
    lookup[filename] = { features: features({ width, height }) as never };
